...
// Build base query
$sql = "SELECT * FROM household_records WHERE 1=1";
...
if (isset($_GET['citizenship']) && !empty($_GET['citizenship'])) {
    $sql .= " AND citizenship = ?";
    $count_sql .= " AND citizenship = ?";
    $params[] = $_GET['citizenship'];
}

if (isset($_GET['occupation']) && !empty($_GET['occupation'])) {
    $sql .= " AND occupation LIKE ?";
    $count_sql .= " AND occupation LIKE ?";
